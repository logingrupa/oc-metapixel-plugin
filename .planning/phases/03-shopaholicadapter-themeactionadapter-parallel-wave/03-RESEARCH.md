...
5. **RESOLVED:** `EventLog.payload` read shape — DB::table raw string vs Eloquent `$jsonable` (Pitfall 8). Phase 3 locks DB::table + explicit `json_decode` (matches D-09 spirit). The `EventLog` model retains `$jsonable = ['payload']` for write-path convenience; the read path in `components/EventPixel.php` (Plan 03-08) uses raw DB::table + explicit `json_decode` for D-09 compliance.
