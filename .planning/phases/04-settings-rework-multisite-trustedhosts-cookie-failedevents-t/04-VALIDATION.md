...
| 04-02-04 | 02 | 2 | HOST-04 | T-04-HOST-01 | `HostIndexResolver::resolve(string $sHost): ?int` returns 1 for apex, 2 for `www.`, 2 for `shop.acme.co.uk`, correct subdomain depth for IDN `xn--bcher-kva.example`. Naive `explode('.')` is banned (PHPStan disallowed-calls). | unit | `composer test -- --filter=HostIndexResolverTest` | ❌ W0 | ⬜ pending |
...
| 04-04-01 | 04 | 4 | FAIL-01 | — | `Backend\Controllers\FailedEvents` ListController shows columns event_id, event_name, adapter_type, http_status, attempts, created_at, graph_error snippet with filter widgets event_name + adapter_type + date range. | feature | `composer test -- --filter=FailedEventsListControllerTest` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/Unit/Host/HostIndexResolverTest.php` — stubs for HOST-04
