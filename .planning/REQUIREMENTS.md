...
- [ ] **SKEL-01**: `Plugin.php` registers all event listeners in `boot()` (Cart `shopaholic.cart.add`, CartPosition `model.afterUpdate`/`model.afterDelete`, Wishlist `ExtendProductComponent::extend` + `addDynamicMethod('onMetaTrackAddToWishList')`, Order `shopaholic.order.created` + `model.afterUpdate`, User `eloquent.created: Lovata\Buddies\Models\User`). `plugin.yaml` declares plugin metadata + requirements.
...
- [ ] **SKEL-06**: `lang/{en,lv,ru}/lang.php` RainLab.Translate-compatible scaffolding for Settings labels (content left empty or stubbed; full translations in Phase 5).
...
| SKEL-02 | Phase 2 | Pending |
| SKEL-03 | Phase 2 | Pending |
| SKEL-04 | Phase 2 | Pending |
| SKEL-05 | Phase 2 | Pending |
| SKEL-06 | Phase 2 | Pending |
