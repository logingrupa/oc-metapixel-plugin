...
4. On the **Hosts & Cookies** tab, fill in:
   ...
   * **Set _fbp / _fbc cookies server-side** — turn this off if your theme already writes these cookies, or if a consent banner must gate them until opt-in. `_fbc` is built from `?fbclid` on the click landing request itself, so that request's CAPI events already carry it, and every new click id replaces the stored value. A click id older than the 90 day cookie lifetime is left out of the Conversions API payload, since Meta reports such values as stale.
